...
dir_model = dir_summary+ MODEL + '_' + current_time + 'loss'
...
for epoch in range(args.epochs):
    ...
    for i, data in tqdm(enumerate(train_loader)):
        ...
        lb_im = lb_im.transpose((0, 2, 3, 1))
        

        rgb_ima = np.zeros_like(lb_im)
        for j in range(outputs.shape[0]):
            rgb_ima[j] = lab2rgb(lb_im[j])
        
        perceptual_crit = 0.0
        rgb_ima = rgb_ima.transpose((0, 3, 1, 2))
        

        original_ima = original.permute(0, 3, 1, 2)

        rgb_ima = torch.from_numpy(rgb_ima).float().to(device)
        original_ima = original_ima.float()

        for j in range(outputs.shape[0]):
            perceptual_crit += perceptual_criterion(rgb_ima[j][np.newaxis, :], original_ima[j][np.newaxis, :])

        perceptual_crit = perceptual_crit / outputs.shape[0]

        #########################################################
        # End Perceptual Loss
        #########################################################
        
        loss = weight_mse*criterion(outputs, ab) + weight_perceptual*perceptual_crit
        loss.backward()
        optimizer.step() 
        running_loss += loss.item()
        perp_loss += perceptual_crit.item()
        ...
        if not os.path.exists(f'output_images/epoch_{epoch}'):
            os.makedirs(f'output_images/epoch_{epoch}') 

        psnr_sum = 0.0
        for j in range(outputs.shape[0]):
            lab_image[j] = lab2rgb(lab_image[j])
