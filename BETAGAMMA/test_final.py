...
parser.add_argument('--resume', type=str, default='BetaGamma/gamma_2023-11-26_03-05-37loss/chkpt_10/cp-0.pth', #TODO: CHANGE THIS TO THE PATH OF THE MODEL YOU WANT TO TEST
                    help='file from which to resume training')
...
current_time = "2023-11-26_03-05-37loss" #TODO: CHANGE THIS TO THE CURRENT TIME and if it is loss or not
...
for i, data in tqdm(enumerate(val_loader)):
    L, ab, input, original = data[0]['L'].to(device), data[0]['ab'].to(device), data[1].to(device), data[2]
    ...
    nuevoL = (L+1.0)*50.0
    nuevoAB = (outputs)*128.0
    lab_image = torch.cat((nuevoL, nuevoAB), dim=1)
    lab_image = lab_image.cpu().detach().numpy()
    lab_image = lab_image.transpose((0, 2, 3, 1))
    
    rgb_image = np.zeros_like(lab_image)
    for j in range(outputs.shape[0]):
        rgb_image[j] = lab2rgb(lab_image[j])
        
    mse = torch.mean((torch.from_numpy(lab_image) - original) ** 2)
    psnr = 20 * math.log10(255 / math.sqrt(mse.item()))
    lista_psnr.append(psnr)
    epoch_psnr += psnr

val_loss = val_loss/(i+1)
mean_psnr = np.mean(lista_psnr)
print('[%d, %5d] val_loss: %.3f' % (i + 1, i + 1, val_loss))
print('[%d, %5d] PSNR: %.3f' % (i + 1, i + 1, mean_psnr))
with open(f'{dir_model}/output_test.txt', 'a') as f:
    print("[Val loss: {:.4f}] [PSNR: {:.4f}]".format(val_loss, mean_psnr), file=f)